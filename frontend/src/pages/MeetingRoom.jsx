...
export default function MeetingRoom() {
  ...
  const { currentMeeting, messages, selectMeeting, endMeeting } =
    useMeetingStore();
  ...
  // Derived State
  const participantStaff =
    currentMeeting?.participants
      ?.map((p) => staff.find((s) => s.id === p.staff_id))
      .filter(Boolean) || [];
  ...
  const { messagesEndRef, chatContainerRef } = useMeetingScroll(
    messages,
    thinkingStaff,
  );
  const { providers, isLoadingProviders, loadProviders } =
    useMeetingProviders();
    ...
    <div className="flex">
      ...
      {showEndModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-neutral-900 rounded-2xl p-8 max-w-lg w-full shadow-2xl border border-transparent dark:border-neutral-800">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">
              Generate Meeting Artifacts
            </h2>
            <p className="text-gray-500 dark:text-neutral-400 mb-8 text-sm">
              Select an intelligence to synthesize the meeting results and
            ...
            {summaryConfig.provider === "ollama" && (
              <div className="mb-6">
                ...
                {isLoadingProviders ? (
                  ...
                  providers.ollama_models.length > 0 ? (
                  <select
                    className="input"
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                disabled={
                  isEnding ||
                  (summaryConfig.provider === "ollama" && !summaryConfig.model)
                }
              >
                {isEnding ? (
                  <>
