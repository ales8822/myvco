...
export default function ChatBubble({ message, participantInfo, onResend }) {
  ...
  const handleResend = async () => {
    if (
      !confirm(
        "Resending this message will delete all subsequent messages. Continue?",
      )
    ) {
      return;
    }
  ...
  const CodeBlock = ({ node, inline, className, children, ...props }) => {
    ...
    if (inline) {
      return (
        <code
          className={`px-1.5 py-0.5 rounded text-sm font-mono ${isUser ? "bg-primary-700 text-white" : "bg-gray-100 dark:bg-neutral-800 text-gray-800 dark:text-neutral-200 border border-gray-200 dark:border-neutral-700"}`}
          {...props}
        >
          {children}
        </code>
      ...
      <div
        ...
        ${
          isUser
            ...
            : "bg-white dark:bg-neutral-900 border border-gray-200 dark:border-neutral-800 text-gray-800 dark:text-neutral-100 rounded-tl-none shadow-sm"
        }`}
      >
        {/* Header */}
        <div className="text-xs opacity-70 mb-1 flex justify-between gap-4 items-center">
            ...
            {participantInfo && (
              <span className="text-xs opacity-60">
                {participantInfo.department_name &&
                  `📂 ${participantInfo.department_name} • `}
                {participantInfo.llm_provider === "ollama" &&
          ...
          <div className="flex items-center gap-2">
            ...
            {(isHovered || isEditing) && (
              <div
                className={`flex gap-1 ${isUser ? "text-white/80" : "text-gray-500"}`}
              >
                <button
                  onClick={handleCopy}
