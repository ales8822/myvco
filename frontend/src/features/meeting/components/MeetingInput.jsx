        ...
        <div className="bg-white dark:bg-neutral-900 border-t border-gray-200 dark:border-neutral-800 p-6 relative transition-colors">
            {showImageUpload ? (
                ...
                <form onSubmit={handleSendMessage} className="space-y-4">
                    <div>
                        <label className="label text-gray-500 dark:text-neutral-500 text-[10px] uppercase font-bold tracking-widest">Responding Staff</label>
                        <select className="input h-10 py-0" value={selectedStaffId || ''} onChange={(e) => setSelectedStaffId(parseInt(e.target.value))} required>
                        <option value="" className="dark:bg-neutral-900">Select staff member</option>
                        {participantStaff.map((member) => <option key={member.id} value={member.id} className="dark:bg-neutral-900">{member.name} - {member.role}</option>)}
                    </select>
                    </div>
                    <div className="flex gap-3 relative items-end">
                        {/* Autocomplete Dropdown */}
                        {showMentionDropdown && filteredMentions.length > 0 && (
<div className="absolute bottom-full left-0 mb-3 w-80 bg-white dark:bg-neutral-800 border border-gray-200 dark:border-neutral-700 rounded-xl shadow-2xl z-50 max-h-64 overflow-y-auto backdrop-blur-md">                                {filteredMentions.map((mention, index) => (
                                    <div
                                    key={mention.id}
                                ...
                                >
                                    <div className="w-10 h-10 flex-shrink-0 bg-gray-100 dark:bg-neutral-900 rounded-lg flex items-center justify-center overflow-hidden border border-gray-200 dark:border-neutral-700">
                                        ...
                                        <div className="font-bold text-sm text-gray-900 dark:text-white truncate">{mention.display}</div>
                                        {mention.description && <div className="text-[10px] text-gray-500 dark:text-neutral-400 truncate uppercase mt-0.5 tracking-tight">{mention.description}</div>}
                                    </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        <textarea
                            ref={inputRef}
className="input flex-1 min-h-[46px] max-h-48 py-3 resize-none overflow-y-auto shadow-inner bg-gray-50 dark:bg-neutral-800 border-gray-200 dark:border-neutral-700 focus:ring-primary-500"                            value={inputMessage}
...
<button type="button" onClick={() => setShowImageUpload(true)} className="h-[46px] px-4 bg-gray-100 dark:bg-neutral-800 text-gray-500 dark:text-neutral-400 rounded-xl border border-gray-200 dark:border-neutral-700 hover:bg-gray-200 dark:hover:bg-neutral-700 transition-all active:scale-95" disabled={isStreaming}>🖼️</button>                        
                        <button 
                        type="button" 
                    ...
                    >
                        ...
                        <button type="submit" className="btn-primary h-[46px] px-6 !rounded-xl shadow-lg shadow-primary-500/20" disabled={!selectedStaffId || isStreaming}>Send</button>
                    )}
                        
<button type="button" onClick={handleAskAll} className="h-[46px] px-4 bg-secondary-600 hover:bg-secondary-700 text-white rounded-xl font-bold text-sm transition-all shadow-md active:scale-95" disabled={isStreaming || participantStaff.length === 0}>Ask All</button>
                        ...
                        {showPathInput && (
                            <div className="absolute bottom-full right-0 mb-3 bg-white dark:bg-neutral-900 p-4 rounded-2xl shadow-2xl border border-gray-100 dark:border-neutral-800 flex flex-col gap-4 w-80 animate-in fade-in slide-in-from-bottom-2 z-50">
                                <div className="flex justify-between items-center">
                                    <label className="text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-neutral-500">Target Path</label>
                                    <button 
                                        type="button" 
                                        onClick={openBrowser}
                                ...
                                <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center">
                                    <div className="bg-white dark:bg-neutral-800 rounded-lg w-96 max-h-[80vh] flex flex-col shadow-2xl border border-transparent dark:border-neutral-700">
                                        <div className="p-3 border-b border-gray-200 dark:border-neutral-700 font-bold bg-gray-50 dark:bg-neutral-700 flex justify-between items-center text-gray-900 dark:text-white">
                                            ...
                                            <button onClick={() => setShowBrowser(false)} className="text-gray-500 dark:text-neutral-400 hover:text-gray-700 dark:hover:text-white">✕</button>
                                        </div>
                                        ...
                                        <div className="flex-1 overflow-y-auto p-2">
                                            <div className="mb-2 flex flex-wrap gap-2">
                                                {drives.map(d => (
                                                    <button key={d} onClick={() => loadDirectory(d)} className="px-2 py-1 bg-gray-200 dark:bg-neutral-600 text-xs rounded hover:bg-gray-300 dark:hover:bg-neutral-500 flex items-center gap-1 text-gray-900 dark:text-neutral-200 transition-colors">
                                                        <HardDrive size={12} /> {d}
                                                    </button>
                                                ))}
                                            </div>
                                        ...
                                        <div className="p-3 border-t border-gray-200 dark:border-neutral-700 flex justify-end gap-2">
                                            <button onClick={() => setShowBrowser(false)} className="px-3 py-1 text-sm text-gray-600 dark:text-neutral-300 hover:bg-gray-100 dark:hover:bg-neutral-700 rounded transition-colors">Cancel</button>
...
className={`h-[46px] px-4 rounded-xl flex items-center gap-2 font-bold text-sm transition-all active:scale-95 ${showPathInput ? 'bg-red-500 text-white' : 'bg-secondary-600 hover:bg-secondary-700 text-white shadow-md'}`}                                disabled={participantStaff.length < 1}
                                title="Start Autonomous Round Table. Click to set path."
                            >
                                {showPathInput ? <span>🚀 GO</span> : <><FolderCode size={18} /> Auto</>}
                            </button>
                        </div>
